...
function max_C_auction_cluster{T <: AbstractFloat}(pM::Array{T, 1},
    ...
    for kk in 1:maxLabel
        ...
        if rct == 1 && cct == 1
            ...
            if typeof(compsum) <: ComparisonSummary
                r2c[clusterRows], c2r[clusterCols], rowCosts[clusterRows], colCosts[clusterCols], λ = scaling_forward_backward(weightMat[clusterRows, clusterCols], rowCosts[clusterRows], colCosts[clusterCols], ε0, εfinal, εscale, λ, check = true)
            elseif typeof(compsum) <: SparseComparisonSummary
                r2c[clusterRows], c2r[clusterCols], rowCosts[clusterRows], colCosts[clusterCols], λ  = scaling_forward_backward(full(weightMat[clusterRows, clusterCols]), rowCosts[clusterRows], colCosts[clusterCols], ε0, εfinal, εscale, λ, check = true)
            else
                error("Unexpected type for compsum")
            end
